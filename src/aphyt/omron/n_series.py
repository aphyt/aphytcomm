...
class AsyncNSeries:
    ...
    async def _get_instance_from_variable_name(self, variable_name: str):
        ...
        data_type_code = b''
        response = None
        # Split on square brackets and period characters to get tokens of derived data types
        derived_data_type_variable_name_split = list(filter(None, re.split(r'[\[\].]', variable_name)))
        try:
            response = await self.connected_cip_dispatcher.get_attribute_all_service(request_path)
            data_type_code = response.reply_data[4:5]
        except CIPException as e:
            """ 
            This CIP command can have an exception when it is a derived data type, and should be passed along
            to be processed in the else statement below, or it doesn't exist and should reraise the exception
            """
            if len(derived_data_type_variable_name_split) > 1:
                pass
            else:
                raise e
        if data_type_code == CIPStructure.data_type_code():
            ...
            super_instance = await self._get_instance_from_variable_name(derived_data_type_variable_name_split[0])
            for token in derived_data_type_variable_name_split[1:]:
                if token.isnumeric():
