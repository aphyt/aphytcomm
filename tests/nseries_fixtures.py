...
class BaseNSeries(unittest.TestCase):
    ...
    def test_derived_data_type_variable_name_spec(self):
        variable_name = 'test_Sequences[0].Move_List[0].Position'
...
class AsyncBaseNSeries(unittest.TestCase):
    # def __init__(self):
    eip_instance = None

    async def test_check_variable_dictionary(self):
        print(self.eip_instance.connected_cip_dispatcher.variables)

    async def test_plc_string_array_change(self):
        variable_string = 'test_DestinationStringArray'
        string_source_variable_string = 'test_StringSource'
        array_index_variable_string = 'test_StringArrayIndex'
        await self.eip_instance.write_variable(array_index_variable_string, 9)
        await self.eip_instance.write_variable(string_source_variable_string, 'Hello World')
        array_value = await self.eip_instance.read_variable(variable_string)
        self.assertEqual(array_value[9], 'Hello World')
        self.eip_instance.write_variable(string_source_variable_string, '')
        array_value = await self.eip_instance.read_variable(variable_string)
        self.assertEqual(array_value[9], '')

    async def test_string_array_read_write(self):
        variable_string = 'test_StringArray'
    ...
    async def test_boolean_read_write(self):
        variable_string = 'test_Bool'
        await self.eip_instance.write_variable(variable_string, False)
        reply = await self.eip_instance.read_variable(variable_string)
        self.assertEqual(reply, False)
        await self.eip_instance.write_variable(variable_string, True)
        reply = await self.eip_instance.read_variable(variable_string)
        self.assertEqual(reply, True)
        await self.eip_instance.write_variable(variable_string, False)
        reply = await self.eip_instance.read_variable(variable_string)
    ...
    async def test_array_of_array(self):
        variable_name = 'test_Sequences'
        result = await self.eip_instance.read_variable(variable_name)
        await self.eip_instance.write_variable(variable_name, result)
    ...
    async def test_derived_data_type_variable_name_spec(self):
        variable_name = 'test_Sequences[0].Move_List[0].Position'
        await self.eip_instance.write_variable(variable_name, 44.444)
        result = await self.eip_instance.read_variable(variable_name)
        self.assertAlmostEqual(result, 44.444)
        await self.eip_instance.write_variable(variable_name, 0.0)
        result = await self.eip_instance.read_variable(variable_name)
        self.assertAlmostEqual(result, 0.0)
